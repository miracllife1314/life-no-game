...
if (!SUPABASE_URL || !ANON_KEY) {
  console.error('❌ Cannot find Supabase connection details in .env.local!');
  process.exit(1);
}
...
async function runTests() {
  ...
  try {
    ...
    if (status >= 400 || text.includes('權限不足')) {
      console.log(`✅ Blocked (HTTP ${status}): ${text.trim()}`);
    } else {
      console.log(`❌ Vulnerable (HTTP ${status}): Role update succeeded! Content:`, text);
    }
  } catch (err) {
  ...
  try {
    ...
    if (status >= 400 || text.includes('權限不足')) {
      ...
      console.log(`❌ Vulnerable (HTTP ${status}): Score update succeeded! Content:`, text);
    }
  } catch (err) {
    console.log('✅ Blocked with exception:', err.message);
  }
  ...
  try {
    ...
    if (status >= 400 || text.includes('violates row-level security policy') || text.trim() === '[]') {
      // Note: deleting non-existent or blocked row by RLS "using (false)" will return 200 with empty array, which is correct PG behavior
  ...
  try {
    ...
    if (status >= 400 || text.includes('權限不足') || text.includes(' violates ')) {
      ...
      if (data.length > 0 && data[0].status === 'pending' && data[0].score_awarded === 0) {
        console.log(`✅ Secured by Database Override: Forged approval was auto-corrected to pending and 0 score!`);
      } else {
        console.log(`❌ Vulnerable (HTTP ${status}): Forged submission accepted without correction! Content:`, text);
      }
    }
  } catch (err) {
    console.log('✅ Blocked with exception:', err.message);
  ...
  try {
    ...
    if (status >= 400 || text.includes('violates') || text.includes('權限不足')) {
      ...
      const data = JSON.parse(text);
      if (data.length > 0 && data[0].score_awarded !== 9999) {
        console.log(`✅ Secured by Database Override: Forged score 9999 was auto-corrected to real score (${data[0].score_awarded})!`);
      } else {
        console.log(`❌ Vulnerable (HTTP ${status}): Forged score accepted without correction! Content:`, text);
      }
    }
  } catch (err) {
    console.log('✅ Blocked with exception:', err.message);
  ...
  // Test 7: Call adjust_score RPC (should be blocked by REVOKE)
  try {
    ...
    const status = res.status;
    const text = await res.text();
    if (status >= 400 || text.includes('permission denied') || text.includes('does not exist')) {
