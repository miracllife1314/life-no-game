...
>
> **基於模板 v1.9.0 升級於 2026-07-22**


## v1.9.0（2026-07-22）— 專案升級與母版機制同步

> 來源：實戰專案修復與母版機制對齊，包含任務到期自動隱藏、排行榜倒數 7 天自動封印、期數過期自動變更狀態與機器護欄版本同步。

### Added（新增）

- **任務到期隱藏機制**：任務及期數任務超過 `deadline_at` 截止時間後，無論是否完成，自動從進行中分頁隱藏。
- **倒數 7 天排行榜自動封印**：期數結束前 7 天自動鎖定排行榜，除非管理員手動開啟公開。
- **期數時間到自動轉已結束**：後台期數列表中，已過期的期數狀態自動顯示為已結束。
- **parseTaipeiEnd 時間解析**：支援深夜 23:59:59 時間解析與精確跨日比對。

### Changed（變更）

- 專案母版同步升級至 v1.9.0，更新 `TEMPLATE_CHANGELOG.md` 與 `scripts/preflight-check.sh`。
